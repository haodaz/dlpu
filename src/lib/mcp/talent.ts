...
// ── 模块级缓存：固定映射表只查一次 ──
let _selectionCachePromise: Promise<Map<string, string>> | null = null;
const _instituteCache = new Map<string, string>(); // slug → name, 累积缓存

async function getSelectionMap(client: any, token?: string): Promise<Map<string, string>> {
  if (!_selectionCachePromise) {
    _selectionCachePromise = (async () => {
      ...
      try {
        ...
        for (const opt of res.items || []) {
          if (opt.value != null) map.set(String(opt.value), opt.name);
        }
      } catch (e) {
        console.warn('[TalentService] SelectionOptions cache load failed:', e);
        _selectionCachePromise = null; // 失败时允许重试
      }
      return map;
    })();
  }
  return _selectionCachePromise;
}

async function getInstituteNames(slugs: string[], client: any, token?: string): Promise<Map<string, string>> {
  const result = new Map<string, string>();
  const uncachedSlugs: string[] = [];
  for (const slug of slugs) {
    const cached = _instituteCache.get(slug);
    if (cached) {
  ...
  if (uncachedSlugs.length > 0) {
    try {
      const slugConditions = uncachedSlugs.map(slug => ({
        leaf: { field: 'slug', comparator: 'ilike', value: slug }
      }));
      const res = await client.dashGenericSearch({
        model: 'CRMInstitute',
        condition: JSON.stringify({ logic_operator: '|', children: slugConditions }),
        fields: ['id', 'name', 'slug'],
        limit: 500,
      }, token);
      for (const inst of (res.items || []) as Array<{ slug: string; name: string }>) {
        if (inst.slug) {
          _instituteCache.set(inst.slug, inst.name);
          result.set(inst.slug, inst.name);
        }
      }
    } catch (e) {
      console.warn('[TalentService] CRMInstitute cache load failed:', e);
    }
  }
  return result;
}
...
export class TalentAuditService {
  ...
  async searchTalents(query: string, limit: number = 10, userToken?: string): Promise<CRMTalentPerson[]> {
    ...
    // 1.5 国籍/国家映射（模块级缓存，首次加载后永不再查）
    const selectionMap = await getSelectionMap(client, token);
    ...
    const instituteMap = allSchoolSlugs.size > 0
      ? await getInstituteNames(Array.from(allSchoolSlugs), client, token)
  ...
  /**
   * 轻量版搜索：只拉 edu/work/award 3个维度 + 专利/论文数量（不拉详情）
   * 用于消歧阶段，MCP 调用次数大幅减少
   */
  async searchTalentsLite(query: string, limit: number = 10, userToken?: string): Promise<CRMTalentPerson[]> {
    const client = mcpToolsDataPlatform;
    const token = userToken || process.env.VISIONSQUARE_AUTH_BEARER;

    // Step 1: 基础表查询（复用 searchTalents 的条件构造逻辑）
    let cleanQuery = query.trim();
    cleanQuery = cleanQuery.replace(/^(?:Dr\.|Dr|Prof\.|Prof|Professor|Mr\.|Mr|Ms\.|Ms|Mrs\.|Mrs)\s+/i, '').trim();
    cleanQuery = cleanQuery.replace(/,\s*(?:Ph\.D\.|PhD|M\.D\.|MD|B\.S\.|BS|M\.S\.|MS)$/i, '').trim();
    ...
    if (names.length > 1) {
      ...
      if (isEnglish) {
        ...
        const spaceParts = cleanQuery.replace(/-/g, ' ').split(/\s+/).filter(Boolean);
        if (spaceParts.length === 2) {
          variations.add(`${spaceParts[1]} ${spaceParts[0]}`);
          variations.add(`${spaceParts[1]}${spaceParts[0]}`);
        } else if (spaceParts.length === 3) {
          variations.add(`${spaceParts[2]} ${spaceParts[0]} ${spaceParts[1]}`);
          variations.add(`${spaceParts[2]} ${spaceParts[0]}${spaceParts[1]}`);
    ...
    const res = await client.dashGenericSearch({
      model: 'CRMTalentPerson',
      condition: JSON.stringify({ logic_operator: '|', children: childrenConditions }),
      fields: ['id', 'name', 'name_en', 'birth_date', 'gender', 'nationality', 'is_chinese', 'province', 'email', 'profile_link', 'research_field', 'talent_source_category', 'talent_type', 'introduction', 'notes', 'country_current', 'admin_position', 'position_current', 'school_current', 'workplace_current', 'photo_id.download_url'],
      limit,
    }, token);
    ...
    for (const t of talents) {
      ...
      if (rawCountries != null) {
        const codes = splitTextArray(rawCountries);
        if (codes.length > 0) t.country_current = codes.map(c => selectionMap.get(c) || c).join('、') as any;
      }
    }

    const allSchoolSlugs = new Set<string>();
    for (const t of talents) {
      splitTextArray(t.school_current).forEach(s => allSchoolSlugs.add(s));
    }
    const instituteMap = allSchoolSlugs.size > 0
      ? await getInstituteNames(Array.from(allSchoolSlugs), client, token)
      : new Map<string, string>();
    for (const t of talents) {
      ...
      if (rawSchools != null) {
        const slugs = splitTextArray(rawSchools);
        if (slugs.length > 0) t.school_current = slugs.map(s => instituteMap.get(s) || s).join('、') as any;
      }
    }

    // Step 2: 轻量维度拉取 — 只拉 edu/work/award + 专利/论文计数
    const enrichedTalents = await Promise.all(talents.map(async (t) => {
      const talentId = t.id;
      
      const [eduRes, workRes, awardRes, patentCountRes, paperCountRes] = await Promise.all([
        client.dashGenericSearch({
          model: 'CRMPeEduBackgrounds',
          condition: JSON.stringify({ logic_operator: '&', children: [{ leaf: { field: 'talent_person_id', comparator: '=', value: talentId } }] }),
          fields: ['degree', 'if_highest_degree', 'start_date', 'end_date', 'if_in_progress_vone', 'school_id', 'school_name_cn', 'school_name_en', 'major_name_cn', 'major_name_en', 'highschool_id', 'highschool_area', 'talent_person_id'],
          limit: 100,
        }, token).catch(() => ({ items: [] })),
        ...
        client.dashGenericSearch({
          model: 'VSDPatentInventor',
          condition: JSON.stringify({ logic_operator: '&', children: [
            { leaf: { field: 'talent_id', comparator: '=', value: talentId } },
            { leaf: { field: 'if_delete', comparator: '=', value: false } }
          ] }),
          fields: ['id'],
          limit: 1,
        }, token).catch(() => ({ items: [], total: 0 })),
      ...
      return {
        ...t,
        education_backgrounds: eduRes.items || [],
        work_experiences: (workRes.items || []).map((w: any) => {
          const rawCountry = w.country;
          if (rawCountry != null) {
            const codes = splitTextArray(rawCountry);
            if (codes.length > 0) w.country = codes.map((c: string) => selectionMap.get(c) || c).join('、');
          }
          return w;
        }),
        award_experiences: awardRes.items || [],
        ...
        papers: [],
        // 消歧评分用的计数字段
        _patent_count: (patentCountRes as any).total ?? (patentCountRes.items?.length || 0),
        _paper_count: (paperCountRes as any).total ?? (paperCountRes.items?.length || 0),
      } as unknown as CRMTalentPerson;
    }));
  ...
  /**
   * 对单个人才拉取完整的专利/论文/基金项目详情
   * 在消歧选出 top1 后调用，避免对所有候选人都拉详情
   */
  async enrichTalentFull(talent: CRMTalentPerson, userToken?: string): Promise<CRMTalentPerson> {
    const client = mcpToolsDataPlatform;
    const token = userToken || process.env.VISIONSQUARE_AUTH_BEARER;
    const talentId = talent.id;

    const [proFunRes, patentInventorRes, paperAuthorRes] = await Promise.all([
      client.dashGenericSearch({
        model: 'CRMPeProFunExperiences',
        condition: JSON.stringify({ logic_operator: '&', children: [{ leaf: { field: 'talent_person_id', comparator: '=', value: talentId } }] }),
        fields: ['if_first_person', 'pe_work_experiences_id', 'program_funding_id', 'program_name', 'program_number', 'talent_person_id', 'type', 'year'],
    ...
    let patents: Record<string, unknown>[] = [];
    if (patentInventorRes.items?.length) {
      const items = patentInventorRes.items.slice(0, 20);
      const patentMap = new Map<number, Record<string, unknown>>();
      const patentResults = await Promise.allSettled(items.map(inv =>
        Promise.race([
          client.dashGenericRelationModelSearch({
    ...
    let papers: Record<string, unknown>[] = [];
    if (paperAuthorRes.items?.length) {
      const items = paperAuthorRes.items.slice(0, 20);
      const paperMap = new Map<number, Record<string, unknown>>();
      const paperResults = await Promise.allSettled(items.map(auth =>
        Promise.race([
          client.dashGenericRelationModelSearch({
