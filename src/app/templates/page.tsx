...
const templates = [
  ...
  { id: 'T16', name: '持续改进与大纲演进 (存证)', status: 'active', desc: '根据架构设计决策，本项不再要求填报，由 AI 直接对比 T11(课程大纲) 的历史版本自动生成。', tags: ['底层自动存证'] },
  { id: 'T17', name: '教学资源清单与使用台账 (存证)', status: 'active', desc: '根据最新架构梳理，该项已在我们搭建 T10 时超前完成，核心资产验证已全面打通。', tags: ['底层自动存证'] },
  { id: 'T18', name: '行业就业率与用人单位满意度', status: 'active', desc: '基于就业系统与爬虫，验证毕业生对口就业率及起薪、满意度。', tags: ['使命型核心'] },
                ...
                <List.Item.Meta
                  title={
                    <div className="flex items-center gap-3 mb-2 mt-1 flex-wrap">
                      ...
                      {item.tags?.map((tag) => (
                        <Tag key={tag} color={tag.includes('使命型') ? 'cyan' : tag.includes('未来型') ? 'purple' : 'default'} className="m-0 font-bold">{tag}</Tag>
                      ))}
                    </div>
                  }
