import { TalentAuditService } from '@/lib/mcp/talentV1';
      ...
      async start(controller) {
        ...
        try {
          ...
          try {
            ...
            if (journalEntry && journalEntry.ai_report) {
...
返回严格的JSON，格式如下：
{
  ...
  "fallback_queries": ["如果首选名字搜不到，应该尝试的英文名或别名搜索词，比如 ['Andrew Ng', 'Ng Andrew']。最多3个。"]
          ...
          sendEvent('log', { step: 'pingfang', message: `🔍 [第一阶段] 正在检索平方数据底座: ${pingfangQuery}...` });
          let pingfangCandidates: any[] = [];

          try {
            pingfangCandidates = await talentService.searchTalents(pingfangQuery, 5);
          } catch (e) {
             sendEvent('log', { step: 'pingfang', message: `⚠️ 平方检索失败: ${e}` });
          }

          // 智能变体兜底：首选匹配没找到，或找到但全机构未命中 → 统一走 AI 推荐变体（fallback_queries）兜底
          if (pingfangQuery) {
            ...
            if (needFallback) {
              const variants = (aiThinkResult.fallback_queries && aiThinkResult.fallback_queries.length > 0)
                ? aiThinkResult.fallback_queries
                : generatePinyinVariants(pingfangQuery);
              ...
              // ══════════════════════════════════════════
                ...
                if (instMatch > 0.5) {
                  ...
                  await runOrcidFullSearch(orcidToken, {
                    ...
                    topPingfangRecord, en_name, searchName, scholarName: allGatheredData['scholar']?.display_name, aiThinkResult
                  }).then(result => {
          ...
          // ── 【修复 1】优先使用 AI 预检专属 Query，再看 pfConfidence ──
...
`;
            ...
            const diagJson = JSON.parse(diagContent.replace(/```json/g, '').replace(/```/g, '').trim());
